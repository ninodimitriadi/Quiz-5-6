...
class Movies(db.Model):
    ...
    def __str__(self):
        return f'სახელი {self.Name}, წელი: {self.Year}, რეიტინგი: {self.Rating}'
    ...
    if request.method == 'POST':
        ...
        if username == "" or password == "":
            ...
            if len(password) >= 8:
                ...
                flash("სიგრძე არასაკმარისია")
        if up_letter and low_letter and digits and sp_ch:
            session['username'] = username
            return redirect(url_for('home'))
        else:
            flash('გთხოვთ მიუთითოთ ისეთი პაროლი, რომელიც შეიცავს დიდ და პატარა ასოებს ასევე რიცხვებსა და სიმბოლოებს')
